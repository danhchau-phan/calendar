...
export default function CalendarEvent({eventLength, eventStartDayId, dayId, title = "(No title)", savedEvent = true, eventPlacement = 0}: CalendarEventProps) {
  ...
  return <>
    {isMultiRowEvent ? <>
      ...
      <div className={clsx("event", !savedEvent && "shadow-xl z-10")} style={{ 
        ...
        gridRowStart: eventPlacement + 1
      }}>{title}</div> :
      dayId > actualEventStartDayId && dayId % NUMBER_OF_DAYS_IN_WEEK === 0 && dayId < eventEndDayId &&
      <div className={clsx("event", !savedEvent && "shadow-xl z-10")} 
            style={{ 
              width: `calc(${100 * (eventEndDayId - dayId >= NUMBER_OF_DAYS_IN_WEEK ? NUMBER_OF_DAYS_IN_WEEK : (eventEndDayId - dayId) % NUMBER_OF_DAYS_IN_WEEK)}% - 9px)`,
              gridRowStart: eventPlacement + 1
      }}
      >{title}</div>}
    </>: dayId === actualEventStartDayId && 
    <div className={clsx("event", !savedEvent && "shadow-xl z-10")} style={{ width: `calc(${100 * actualLength}% - 9px)`, gridRowStart: eventPlacement + 1}}>{title}</div>}
    </>
}
